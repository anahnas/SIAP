...
def ml_methods(train_df, test_df):
    text_clf_svm = Pipeline([('vect', CountVectorizer(min_df=0, lowercase=True, stop_words=stopwords, max_df=0.80)),
                          ('tfidf', TfidfTransformer()),
                          ('clf-svm', LinearSVC())])

    _ = text_clf_svm.fit(train_df['preprocessed_plots'].values, train_df['tags'].values)
    predicted_svm = text_clf_svm.predict(test_df['preprocessed_plots'].values)
    ...
    _ = text_clf_nb.fit(train_df['preprocessed_plots'].values, train_df['tags'].values)
    predicted_nb = text_clf_nb.predict(test_df['preprocessed_plots'].values)
